...
resource "aws_cloudwatch_metric_alarm" "alb_4xx_alarm" {
  ...
  // Aggregates 4xx responses each minute
  evaluation_periods  = 1
  period              = 60
  statistic           = "Sum"

  // Triggers alarm if 10 or more 4xx errors in one period. This value is low here for testing purposes, it would be higher in a production environment.
  comparison_operator = "GreaterThanOrEqualToThreshold"
  threshold           = 10

  dimensions = { LoadBalancer = var.alb_arn_suffix }                 // Specify which ALB to monitor.
  alarm_actions = [aws_sns_topic.alerts.arn]                         // When alarm is triggered : sends email via SNS.
}
